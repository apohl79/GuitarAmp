...
void PopulateDriverSpecificControls(HWND hwndDlg) {
    ...
    for (int i = 0; i < gAudioInputDevs.size(); i++) {
        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IN_DEV, CB_ADDSTRING, 0,
                           (LPARAM)GetAudioDeviceName(gAudioInputDevs[i]).c_str());

        if (!strcmp(GetAudioDeviceName(gAudioInputDevs[i]).c_str(), gState->mAudioInDev)) indevidx = i;
    }

    for (int i = 0; i < gAudioOutputDevs.size(); i++) {
        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_OUT_DEV, CB_ADDSTRING, 0,
                           (LPARAM)GetAudioDeviceName(gAudioOutputDevs[i]).c_str());

        if (!strcmp(GetAudioDeviceName(gAudioOutputDevs[i]).c_str(), gState->mAudioOutDev)) outdevidx = i;
    }
...
void PopulateAudioDialogs(HWND hwndDlg) {
    ...
    LRESULT iovsidx =
        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IOVS, CB_FINDSTRINGEXACT, -1, (LPARAM)gState->mAudioIOVS);
    ...
    LRESULT sigvsidx =
        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_SIGVS, CB_FINDSTRINGEXACT, -1, (LPARAM)gState->mAudioSigVS);
...
bool PopulateMidiDialogs(HWND hwndDlg) {
    if (!gMidiIn || !gMidiOut)
        ...
        LRESULT indevidx =
            SendDlgItemMessage(hwndDlg, IDC_COMBO_MIDI_IN_DEV, CB_FINDSTRINGEXACT, -1, (LPARAM)gState->mMidiInDev);
        ...
        for (int i = 0; i < gMIDIOutputDevNames.size(); i++) {
            SendDlgItemMessage(hwndDlg, IDC_COMBO_MIDI_OUT_DEV, CB_ADDSTRING, 0,
                               (LPARAM)gMIDIOutputDevNames[i].c_str());
        }

        LRESULT outdevidx =
            SendDlgItemMessage(hwndDlg, IDC_COMBO_MIDI_OUT_DEV, CB_FINDSTRINGEXACT, -1, (LPARAM)gState->mMidiOutDev);
    ...
    switch (uMsg) {
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDOK:
                    if (memcmp(gActiveState, gState, sizeof(AppState)) !=
                        0)  // if state is different try to change audio
                    {
                        TryToChangeAudio();
                    }
                ...
                case IDC_COMBO_AUDIO_IN_DEV:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        ...
                        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IN_DEV, CB_GETLBTEXT, idx,
                                           (LPARAM)gState->mAudioInDev);
                ...
                case IDC_COMBO_AUDIO_OUT_DEV:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        ...
                        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_OUT_DEV, CB_GETLBTEXT, idx,
                                           (LPARAM)gState->mAudioOutDev);
                ...
                case IDC_COMBO_AUDIO_IN_R:
                    if (HIWORD(wParam) == CBN_SELCHANGE)
                        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IN_R, CB_SETCURSEL, gState->mAudioInChanR - 1,
                                           0);  // TEMP
                    //                gState->mAudioInChanR = (int) SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IN_R,
                    //                CB_GETCURSEL, 0, 0);
                    break;

                case IDC_COMBO_AUDIO_OUT_L:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        gState->mAudioOutChanL =
                ...
                case IDC_COMBO_AUDIO_OUT_R:
                    if (HIWORD(wParam) == CBN_SELCHANGE)
                        SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_OUT_R, CB_SETCURSEL, gState->mAudioOutChanR - 1,
                                           0);  // TEMP
                    //                gState->mAudioOutChanR = (int) SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_OUT_R,
                    //                CB_GETCURSEL, 0, 0);
                    break;
                ...
                case IDC_COMBO_AUDIO_SIGVS:
                    ...
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        ...
                        if (atoi(kIOVSOptions[iovsidx].c_str()) <
                            atoi(kSIGVSOptions[sigvsidx].c_str()))  // if iovs < sigvs
                        {
                            SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IOVS, CB_GETLBTEXT, iovsidx,
                                               (LPARAM)gState->mAudioIOVS);
                            ...
                            SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_IOVS, CB_GETLBTEXT, iovsidx,
                                               (LPARAM)gState->mAudioIOVS);
                            SendDlgItemMessage(hwndDlg, IDC_COMBO_AUDIO_SIGVS, CB_GETLBTEXT, sigvsidx,
                                               (LPARAM)gState->mAudioSigVS);
                        }
                    }
                    break;
                ...
                case IDC_COMBO_MIDI_IN_DEV:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        ...
                        SendDlgItemMessage(hwndDlg, IDC_COMBO_MIDI_IN_DEV, CB_GETLBTEXT, idx,
                                           (LPARAM)gState->mMidiInDev);
                ...
                case IDC_COMBO_MIDI_OUT_DEV:
                    if (HIWORD(wParam) == CBN_SELCHANGE) {
                        ...
                        SendDlgItemMessage(hwndDlg, IDC_COMBO_MIDI_OUT_DEV, CB_GETLBTEXT, idx,
                                           (LPARAM)gState->mMidiOutDev);
            ...
            switch (LOWORD(wParam)) {
                ...
                case ID_ABOUT:
                    if (!gPluginInstance->HostRequestingAboutBox()) {
                        char version[50];
                        sprintf(version, PLUG_MFR "\nBuilt on " __DATE__);
                        MessageBox(hwndDlg, version, BUNDLE_NAME, MB_OK);
                    }
                    return 0;
                case ID_PREFERENCES: {
                    INT_PTR ret =
                        DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG_PREF), hwndDlg, PreferencesDlgProc);
